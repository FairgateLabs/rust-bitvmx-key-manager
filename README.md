...
- **`transactional`** (disabled by default): Enables transactional database operations for atomicity guarantees when storing keys. When enabled, all database operations are wrapped in transactions to ensure consistency. Disable this feature if you need to use the library without transactional support.
...
- **`lamport_idx_check`** (enabled by default): Enables index validation checks for Lamport One-Time Signature  keys. This ensures that each lamport key index is only used once, preventing security vulnerabilities from key reuse. It's highly recommended to keep this enabled for production use.
