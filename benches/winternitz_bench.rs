...
fn criterion_benchmark(_c: &mut Criterion) {
    ...
    for number_of_keys in numbers_of_keys_to_hash {
        criterion.bench_function(
            &format!("Hashing {} Winternitz keys calling everytime the Winternitz Secret", number_of_keys),
            |b| {
                b.iter(|| {
                    for i in 0..number_of_keys {
                        key_manager.derive_winternitz(32, WinternitzType::HASH160, i).unwrap();
                    }
                })
            });
        
        criterion.bench_function(
            &format!("Hashing {} Winternitz keys calling the Winternitz Secret only once", number_of_keys),
            |b| {
                b.iter(|| {
                    key_manager.derive_multiple_winternitz(32, WinternitzType::HASH160, 0, number_of_keys)
                })
            });
    }
