...
use create::{clear_storage, create_key_manager_example};
...
fn sign_verify_musig2_example() {
    ...
    let my_pub_nonces = key_manager
        .get_my_pub_nonces(&aggregated_pubkey, session_id)
        .unwrap();
    println!("My public nonces: {:?}", my_pub_nonces.clone());

    let other_pub_nonces = other_key_manager
        .get_my_pub_nonces(&aggregated_pubkey, session_id)
        .unwrap();
    println!("Other public nonces: {:?}", other_pub_nonces.clone());
    ...
    let my_partial_sigs = key_manager
        .get_my_partial_signatures(&aggregated_pubkey, session_id)
        .unwrap();
    println!("My partial signatures: {:?}", my_partial_sigs.clone());

    // Get the partial signatures for the message
    let other_partial_sigs = other_key_manager
        .get_my_partial_signatures(&aggregated_pubkey, session_id)
        .unwrap();
    println!("Other partial signatures: {:?}", other_partial_sigs.clone());
    ...
    key_manager
        ...
        .unwrap();
    println!("Save my partial signatures");

    // Do the same for the other participant
    other_key_manager
        .save_partial_signatures(&aggregated_pubkey, session_id, partial_sigs_map.clone())
        .unwrap();
    println!("Save other partial signatures");

    // Step 7: Get the aggregated signature
    let aggregated_signature = key_manager
        .get_aggregated_signature(&aggregated_pubkey, session_id, message_id)
        .unwrap();
    println!("Aggregated signature: {:?}", aggregated_signature);

    // Verify the final signature
    let verification = key_manager
