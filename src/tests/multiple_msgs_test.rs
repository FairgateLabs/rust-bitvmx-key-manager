...
mod tests {
    ...
    fn test_multiple_messages() -> Result<(), anyhow::Error> {
        ...
        let id = "test_id";
        let aggregated_pub_key = key_manager.musig2().new_session(participant_pubkeys.clone(), pub_key_part_1)?;
        ...
        let aggregated_pub_key_2 =
            key_manager2.musig2().new_session(participant_pubkeys.clone(), pub_key_part_2)?;
        ...
        // Add nonces
        let nonces_1 = key_manager.musig2().get_my_pub_nonces(&aggregated_pub_key, id).unwrap();
        let nonces_2 = key_manager2.musig2().get_my_pub_nonces(&aggregated_pub_key_2, id).unwrap();
        ...
        key_manager.musig2().aggregate_nonces(&aggregated_pub_key, id, nonces_map_2.clone())?;
        key_manager2.musig2().aggregate_nonces(&aggregated_pub_key_2, id, nonces_map_1.clone())?;
        ...
        let signature_1 = key_manager.musig2().get_aggregated_signature(&aggregated_pub_key, id, &messages[0])?;
        let signature_2 =
            key_manager2.musig2().get_aggregated_signature(&aggregated_pub_key_2, id, &messages[1])?;
        // Check that signatures length matches number of messages

        let verification_1 =
            key_manager.musig2().verify_final_signature(&messages[0], signature_1, aggregated_pub_key, id)?;

        let verification_2 =
            key_manager2.musig2().verify_final_signature(&messages[1], signature_2, aggregated_pub_key_2, id)?;
