...
impl HashFunction for WinternitzType {
    fn hash(&self, data: &WinternitzHash) -> WinternitzHash {
        let hash = match self {
            WinternitzType::SHA256 => WinternitzHash::new(
                sha256::Hash::hash(&data.to_bytes())
                    .as_byte_array()
                    .to_vec(),
            ),
...
impl WinternitzSignature {
    ...
    pub fn from_hashes_and_digits(
        hashes: &[u8],
        checksummed_digits: &[u8],
        message_length: usize,
        hash_type: WinternitzType,
    ) -> Result<Self, WinternitzError> {
        let hash_size = hash_type.hash_size();
...
impl WinternitzPublicKey {
    ...
    fn push_hash(&mut self, hash: WinternitzHash) -> Result<(), WinternitzError> {
        if hash.len() != self.hash_type.hash_size() {
            return Err(WinternitzError::HashSizeMissmatch(
                hash.len(),
    ...
    pub fn message_size(&self) -> Result<usize, WinternitzError> {
        let message_size = self
            .extra_data
            .as_ref()
            .ok_or(WinternitzError::ExtraDataMissing(
...
impl Winternitz {
    ...
    pub fn generate_public_key(
        ...
        checksum_size: usize,
        derivation_index: u32,
    ) -> Result<WinternitzPublicKey, WinternitzError> {
    ...
    pub fn generate_private_key(
        ...
        let private_key = self.generate_hashes(
            master_secret,
            key_type,
            message_size,
            checksum_size,
    ...
    pub fn sign_message(
        &self,
        message_digits: usize,
        checksummed_message: &[u8],
        private_key: &WinternitzPrivateKey,
    ) -> WinternitzSignature {
        let mut signature = WinternitzSignature::new(message_digits);
